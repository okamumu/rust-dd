...
fn make_ft(dd: &mut BddManager, vars: &[Node], x: &[Vec<usize>]) -> Node {
    let mut f = dd.zero();
...
fn bench_ft1() {
    ...
    let headers = labels
        .into_iter()
        .map(|i| dd.create_header(i, &format!("{}", i)))
        .collect::<Vec<_>>();
    let vars = labels
        .into_iter()
        .map(|i| dd.create_node(headers[i], dd.zero(), dd.one()))
        .collect::<Vec<_>>();
...
fn make_test_data() -> (Vec<usize>, Vec<Vec<usize>>) {
    ...
    let labels = [
        13, 19, 2, 14, 29, 23, 3, 26, 25, 7, 9, 27, 12, 30, 17, 24, 8, 4, 18, 5, 20, 21, 28, 1, 16,
        10, 15, 6, 11, 22,
    ];
    let r = 0.3;
    let gridn = 100;
...
fn bench_ft2() {
    ...
    let mut dd = BddManager::new();
    let headers = (0..labels.len())
        .into_iter()
        .map(|i| dd.create_header(i, &format!("{}", labels[i])))
        .collect::<Vec<_>>();
    let vars = (0..labels.len())
        .into_iter()
        .map(|i| dd.create_node(headers[i], dd.zero(), dd.one()))
        .collect::<Vec<_>>();
