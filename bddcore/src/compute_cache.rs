//! Direct-mapped, lossy computed table for `apply` memoization (CUDD-style).
//!
//! The operation cache is only a *hint*: a miss costs a recomputation, never a
//! wrong answer. So instead of a growing `HashMap` (probing + periodic rehash on
//! every `put`, which dominates `apply` on workloads of many tiny operations),
//! this is a fixed-power-of-two array indexed by `hash(key) & mask`, with a
//! single slot per bucket that is simply overwritten on collision.
//!
...
//! sentinel. Keys are already u32-narrowed by the manager (op code + two ids).
...
impl ComputeCache {
    ...
    /// must be invalidated. `k1`/`k2` are operand ids (`k2 == 0` for `not`, the
    /// always-live zero terminal); `val` is the result id.
